...
def calculate_codon_replicability(df, assays, assay_cols):
    ...
    for k, (assay, assay_col) in enumerate(zip(assays, assay_cols)):    
        #print(assay)
...
def plot_codon_replicability(df, assays=r2_assays, assay_titles=r2_assay_titles, DNA=True, fig_outdir='figures', figname='fig2S2'):
    ...
    assay_cols = [col + '_mean_RPM' for col in assays]
    
    if DNA:
        plot_assays = list(np.append(assays, 'DNA'))
        plot_assay_cols = list(np.append(assay_cols, 'DNA_mean_RPM'))
        shift = 1
    else:
        plot_assays = assays
        plot_assay_cols = assay_cols
        shift = 0

    plot_data_codon_rep, df_codon_agg = calculate_codon_replicability(df, plot_assays, plot_assay_cols)
    ...
    for k, assay in enumerate(assays):
        if k % ncol == 3 or k == (len(assays) - 1):
            gsa = mpl.gridspec.GridSpecFromSubplotSpec(
                2, 2, subplot_spec=gs[k // ncol + shift, k % ncol], hspace=0.0, wspace=0.1, height_ratios=[6, 1], width_ratios=[12,1])
        else:
            gsa = mpl.gridspec.GridSpecFromSubplotSpec(
                2, 1, subplot_spec=gs[k // ncol + shift, k % ncol], hspace=0.0, wspace=0.1, height_ratios=[6, 1])
